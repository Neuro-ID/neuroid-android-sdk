...
/**
 * Triggers the advanced-device (FPJS) capture exactly once, the first time the host
 ...
 * cold start) where no `Activity` is ever created. This avoids issuing an unnecessary FPJS call
 * during those headless operations.
 *
 * This is the only place where the checkThenCaptureAdvancedDevice() should be called.
 */
internal class NIDAdvancedDeviceLifecycleObserver(
