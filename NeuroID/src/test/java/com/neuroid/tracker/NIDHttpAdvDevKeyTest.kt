...
class NIDHttpAdvDevKeyTest {
    ...
    fun test_http_200_ok() {
        ...
        every { mapper.getKey("{\"status\":\"OK\", \"key\":\"g2334asdgawe4535435r=\"}") } returns AdvancedDeviceKey(
            "OK",
            "g2334asdgawe4535435r="
        )
    ...
    fun test_http_200_not_ok() {
        val datasource = mockk<NIDDataStoreManager>()
        every { datasource.saveEvent(any()) } answers {
            mockk<Job>()
        }
        val callBack = mockk<OnKeyCallback>()
        every { callBack.onFailure(any(), any()) } just runs
        ...
        every { connProvider.getConnection(any()) } returns conn
        val mapper = mockk<GsonAdvMapper>()
        every { mapper.getKey("{\"status\":\"notOK\", \"key\":\"g2334asdgawe4535435r=\"}") } returns AdvancedDeviceKey(
            "notOK",
            "g2334asdgawe4535435r="
        )
        val keyService = NIDAdvKeyService()
        ...
        verify(exactly = 1) {
            callBack.onFailure(
                "advanced signal not available: status notOK",
                200
            )
        }
    }
    ...
    fun test_http_404() {
        ...
        verify(exactly = 3) {
            callBack.onFailure(
                "error! response message: big error! method: get",
                404
            )
        }
    }
    ...
    fun test_http_no_response_with_message() {
        ...
        verify(exactly = 3) {
            callBack.onFailure(
                "error! no response, message: big stack trace",
                -1
            )
        }
    }
    ...
    fun test_http_no_response_no_message() {
        ...
        verify(exactly = 3) {
            callBack.onFailure(
                "error! no response, message: some exception",
                -1
            )
        }
    }
